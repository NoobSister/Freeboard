...
public class GalleryRegistAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		String path="c:\\upload";		// 서버컴퓨터 로컬 경로 - 폴더 만들어 놓으세요.
		// 프로젝트 폴더와 관련 없는 경로일때  url 매핑을 server.xml에 설정한다.
	    // String path = request.getServletContext().getRealPath("/image");
		// 			ㄴ 업로드 폴더가 프로젝트 폴더의 하위 경로 image 폴더일 때 사용합니다.

	      
	      int size = 10 * 1024 * 1024;	// 10MB, 최대파일 크기
	      try {
	         ...
	         // 파일을 받아오고 지정된 path에 저장, 저장된 파일 리턴
